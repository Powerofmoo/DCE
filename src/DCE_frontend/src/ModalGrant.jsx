...
const ModalGrant = ({ isOpen, onClose, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const result = await submitEntry({ id, green, text });
    if (result) {
  ...
  const submitEntry = async ({  id, green, text }) => {
    try {
      console.log('Submitting entry:', { id, green, text });
      // Simulate a network request
      var ret = await DCE_authBackend.grant(id, BigInt(green), text);
    ...
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit">Grant Coins</button>
        </form>
